...
@app.route('/water_distribution.html')
def serve_water_distribution():
    """Serve water distribution page explicitly"""
    try:
        return send_from_directory(os.path.dirname(os.path.abspath(__file__)), 'water_distribution.html')
    except Exception as e:
...
def serve_static(filename):
    ...
    try:
        ...
        allowed_extensions = ('.html', '.css', '.js', '.json', '.csv')
        if any(filename.endswith(ext) for ext in allowed_extensions):
            file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), filename)
            # Security: ensure file is within the allowed directory
            if os.path.abspath(file_path).startswith(os.path.dirname(os.path.abspath(__file__))):
